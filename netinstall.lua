...
menuOptions("Select CloverOS edition", {
	"default (Recommended)",
	"soft (Recommended for low storage installs or lightweight.)",
	"turtle (Recommended for turtles)",
	"emulator (Recommended edition for Emulated CC: Tweaked computers)",
}, {
	function()
		edition = "default"
	end,
...
-- Advanced Settings
local theme = nil
menuOptions("Select Theme", { "Default (Black/White)", "Dark (Black/Gray)", "Light (White/Black)", "Colorful" }, {
	function()
		theme = "default"
	end,
	function()
...
local autoLogin = nil
menuOptions("Enable Auto-Login?", { "Yes", "No" }, {
	function()
		autoLogin = true
	end,
	function()
...
menuOptions("Enable Sound Effects?", { "Yes", "No" }, {
	function()
		soundEnabled = true
	end,
	function()
...
menuOptions("Performance Mode", { "Standard", "Low (Reduced Animations)" }, {
	function()
...
elseif edition == "soft" then
	...
	for _, components in ipairs(components) do
		table.insert(fileList, components)
	end
...
print("Installation complete. Applying patches now")
settings.set("theme", theme)
settings.set("autoLogin", autoLogin)
settings.set("soundEnabled", soundEnabled)
settings.set("performanceMode", performanceMode)
settings.save()
sleep(2)
os.reboot()
