...
elseif not getmetatable(expect).__call then
	getmetatable(expect).__call = function(self, ...)
		return self.expect(...)
	end
end
...
local function unbios(path, ...)
	...
	for k in pairs(_G) do
		if not keptAPIs[k] and not userGlobals[k] then
			table.insert(t, k)
		end
	end
...
function cmds.args(t)
	if not bootcfg.args then
		error("config.lua:" .. t.line .. ": args command must come after boot type", 0)
	end
	for i = 1, #t.args do
		bootcfg.args[#bootcfg.args + 1] = t.args[i]
	end
end
...
function cmds.monitor(t)
	if peripheral.hasType then
		assert(
			peripheral.hasType(t.name, "monitor"),
			"peripheral '" .. t.name .. "' does not exist or is not a monitor"
		)
	else
		assert(
			peripheral.getType(t.name) == "monitor",
			"peripheral '" .. t.name .. "' does not exist or is not a monitor"
		)
	end
...
config = setmetatable({
		...
		return function(entry)
			...
			for i = 1, n do
				...
				if (type(c) ~= "table" and type(c) ~= "function") or not c.cmd then
					error(
						"bad command entry #" .. i .. (c == nil and " (unknown command)" or " (missing arguments)"),
						2
					)
				end
	...
	include = function(path)
		...
		if not path:match("^/") then
			path = fs.combine(runningDir, path)
		end
...
local function drawScreen()
	local bbg, bfg =
		hex(select(2, math.frexp(config.boxbackground or config.backgroundcolor))),
...
while true do
	...
	if ev[1] == "timer" and ev[2] == tm then
		...
		if config.timeout == 0 then
			if boot(entry_names[config.defaultentry]) then
				return
			end
		end
		drawEntries()
